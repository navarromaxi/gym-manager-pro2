-- Ejecutar después de classes-foundation.sql.
-- Esta función sólo puede ser llamada por el servidor (service_role), no desde el navegador.
...
begin
  ...
  select * into occurrence_row from public.class_occurrences
    where id = p_occurrence_id and gym_id = p_gym_id for update;
  ...
  if exists (select 1 from public.class_reservations where occurrence_id = occurrence_row.id and member_id = member_row.id) then
  ...
  insert into public.class_reservations (occurrence_id, gym_id, member_id)
    values (occurrence_row.id, p_gym_id, member_row.id);
  return jsonb_build_object('ok', true, 'member_name', member_row.name);
end;
$$;
