      ...
      <Dialog open={dailyDialogOpen} onOpenChange={setDailyDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto border-slate-200 bg-white text-slate-950 [&_input]:border-slate-300 [&_input]:bg-white [&_input]:text-slate-950 [&_textarea]:border-slate-300 [&_textarea]:bg-white [&_textarea]:text-slate-950">
          ...
          <form onSubmit={handleCreateDailyEvent} className="grid gap-4 md:grid-cols-2 [&_label]:text-slate-800">
            ...
            <label className="md:col-span-2 flex cursor-pointer items-start gap-3 rounded-xl !border-emerald-300 !bg-emerald-100 p-4 !text-emerald-950"><Checkbox className="border-emerald-600 bg-white" checked={dailyEventForm.accept_receipts} onCheckedChange={(checked) => setDailyEventForm((current) => ({ ...current, accept_receipts: checked === true }))} /><span className="!text-emerald-950"><b className="!text-emerald-950">Solicitar comprobante</b><span className="mt-1 block text-sm !text-emerald-800">Al reservar, será obligatorio adjuntar una imagen o PDF.</span></span></label>
