...
export function ReportsSection({ members, payments, expenses, gymName }: ReportsSectionProps) {
  ...
  const getFilteredData = () => {
    ...
    switch (timeFilter) {
      ...
      case "last_6_months":
        ...
        filteredPayments = payments.filter((p) => new Date(p.date) >= sixMonthsAgo)
        filteredExpenses = expenses.filter((e) => new Date(e.date) >= sixMonthsAgo)
        break

      case "current_year":
        filteredPayments = payments.filter((p) => new Date(p.date).getFullYear() === currentDate.getFullYear())
        filteredExpenses = expenses.filter((e) => new Date(e.date).getFullYear() === currentDate.getFullYear())
        break

      case "last_year":
        const lastYear = currentDate.getFullYear() - 1
        filteredPayments = payments.filter((p) => new Date(p.date).getFullYear() === lastYear)
        filteredExpenses = expenses.filter((e) => new Date(e.date).getFullYear() === lastYear)
        break
  ...
  const getRenewalStats = () => {
    ...
    const memberPaymentCounts = payments.reduce(
      (acc, payment) => {
    ...
    const renewedMembers = Object.entries(memberPaymentCounts).filter(([_, count]) => count > 1)
    ...
    const notRenewedMembers = members.filter((member) => {
      ...
      return paymentCount <= 1 && (member.status === "inactive" || member.status === "expired")
    })
    const notRenewedCount = notRenewedMembers.length

    // Tasa de renovación
    const totalEligibleForRenewal = renewedCount + notRenewedCount
    const renewalRate = totalEligibleForRenewal > 0 ? (renewedCount / totalEligibleForRenewal) * 100 : 0
  ...
  // Plan distribution
  const planDistribution = members.reduce(
    (acc, member) => {
  ...
  const handleExportReport = () => {
    ...
    link.href = url
    link.download = `reporte-${gymName.toLowerCase().replace(/\s+/g, "-")}-${new Date().toISOString().split("T")[0]}.json`
    link.click()
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Análisis completo del rendimiento de {gymName}</p>
        </div>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">{filteredPayments.length} pagos</div>
            </div>
            <div className="text-center p-4 bg-red-50 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">{filteredExpenses.length} gastos</div>
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{renewalStats.renewedCount}</div>
              <div className="text-sm text-muted-foreground">Socios que Renovaron</div>
              <div className="text-xs text-muted-foreground">Tienen más de 1 pago</div>
            </div>
            <div className="text-center p-4 bg-red-50 rounded-lg">
              <div className="text-2xl font-bold text-red-600">{renewalStats.notRenewedCount}</div>
              <div className="text-sm text-muted-foreground">No Renovaron</div>
              <div className="text-xs text-muted-foreground">Vencidos/Inactivos</div>
            </div>
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{renewalStats.renewalRate}%</div>
              <div className="text-sm text-muted-foreground">Tasa de Renovación</div>
              <div className="text-xs text-muted-foreground">% que renovaron</div>
            </div>
            <div className="text-center p-4 border rounded-lg">
              <div className="text-2xl font-bold">{renewalStats.totalEligible}</div>
              <div className="text-sm text-muted-foreground">Total Elegibles</div>
              <div className="text-xs text-muted-foreground">Para renovación</div>
            </div>
          </div>
              ...
              <div className="flex-1">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${renewalStats.totalEligible > 0 ? (renewalStats.renewedCount / renewalStats.totalEligible) * 100 : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
              <div className="flex-1">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${renewalStats.totalEligible > 0 ? (renewalStats.notRenewedCount / renewalStats.totalEligible) * 100 : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <div className="text-center p-4 border rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{expiredMembers}</div>
            ...
            {upcomingExpirations.length > 0 && (
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-orange-800">Vencimientos Próximos</h4>
                    <p className="text-sm text-orange-600">
                      {upcomingExpirations.length} socios con plan por vencer en los próximos 7 días
                    </p>
                  </div>
                  <Badge className="bg-orange-500 text-white">{upcomingExpirations.length}</Badge>
                </div>
              </div>
            )}
              ...
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-red-600">{overdueMembers.length} socios con pagos vencidos</p>
                  </div>
                  <Badge className="bg-red-500 text-white">{overdueMembers.length}</Badge>
                </div>
              </div>
            )}

            {renewalStats.renewalRate < 50 && renewalStats.totalEligible > 0 && (
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-yellow-800">Baja Tasa de Renovación</h4>
                    <p className="text-sm text-yellow-600">
                      Solo el {renewalStats.renewalRate}% de los socios elegibles renovaron su plan
                    </p>
                  </div>
                  <Badge className="bg-yellow-500 text-white">{renewalStats.renewalRate}%</Badge>
                </div>
              </div>
            )}

            {upcomingExpirations.length === 0 && overdueMembers.length === 0 && renewalStats.renewalRate >= 50 && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center">
                  <div>
                    <h4 className="font-medium text-green-800">Todo en Orden</h4>
                    <p className="text-sm text-green-600">No hay alertas importantes en este momento</p>
                  </div>
                </div>
              </div>
            )}
              ...
              {Object.entries(planDistribution).map(([plan, count]) => (
                <div key={plan} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant="outline">{Math.round((count / members.length) * 100)}%</Badge>
                  </div>
                </div>
              ))}
            </div>
              ...
              {Object.entries(paymentMethodDistribution).map(([method, count]) => (
                <div key={method} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant="outline">{Math.round((count / filteredPayments.length) * 100)}%</Badge>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            {last6MonthsIncome.map((monthData, index) => (
              <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <span className="text-lg font-bold text-green-600">${monthData.income.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </CardContent>
              ...
              <TableBody>
                {upcomingExpirations.map((member) => {
                  const daysUntilExpiration = Math.ceil(
                    (new Date(member.nextPayment).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24),
                  )
                  return (
                    <TableRow key={member.id}>
                      ...
                      <TableCell>
                        <Badge variant={daysUntilExpiration <= 3 ? "destructive" : "secondary"}>
                          {daysUntilExpiration === 0 ? "Hoy" : `${daysUntilExpiration} días`}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  )
