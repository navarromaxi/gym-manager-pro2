...
export function InactiveManagement({ members, setMembers, payments }: InactiveManagementProps) {
  ...
  const updateInactiveLevel = (memberId: string, newLevel: "green" | "yellow" | "red") => {
    setMembers(members.map((member) => (member.id === memberId ? { ...member, inactiveLevel: newLevel } : member)))
  }

  const updateMemberComment = (memberId: string, comment: string) => {
    setMembers(members.map((member) => (member.id === memberId ? { ...member, inactiveComment: comment } : member)))
  ...
  const getMemberPaymentInfo = (memberId: string) => {
    ...
    return {
      totalPaid,
      paymentCount,
      lastPaymentAmount: memberPayments.length > 0 ? memberPayments[memberPayments.length - 1].amount : 0,
    }
  }
  ...
  const getColorBadge = (level?: "green" | "yellow" | "red") => {
    switch (level) {
      ...
      case "yellow":
      default:
        return <Badge className="bg-yellow-500 text-white">En Evaluación</Badge>
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Administra y clasifica socios inactivos (+30 días sin pagar)</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{colorStats.green}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">{colorStats.yellow}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{colorStats.red}</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Socios Inactivos ({filteredInactiveMembers.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                    ...
                    <TableRow key={member.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center">
                          ...
                          <span className="font-medium text-red-600">{daysSinceLastPayment} días</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          <div className="font-medium">${paymentInfo.totalPaid.toLocaleString()}</div>
                      ...
                      <TableCell>
                        <div className="flex space-x-1">
                          <Button
                            ...
                            onClick={() => updateInactiveLevel(member.id, "green")}
                          >
                            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
                          </Button>
                          <Button
                            ...
                            onClick={() => updateInactiveLevel(member.id, "yellow")}
                          >
                            <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                          </Button>
                          <Button
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <p className="text-sm text-green-700">
                Socios que probablemente regresen con una buena estrategia de retención.
              </p>
            </div>
            <div className="p-4 bg-yellow-50 rounded-lg">
              <div className="flex items-center mb-2">
                <div className="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
                <span className="font-medium text-yellow-800">En Evaluación</span>
              </div>
              <p className="text-sm text-yellow-700">
                Socios que requieren análisis adicional para determinar su potencial de retorno.
              </p>
            </div>
            <div className="p-4 bg-red-50 rounded-lg">
              ...
              <p className="text-sm text-red-700">
                Socios que probablemente no regresen. Considerar remover de campañas activas.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Dialog open={isCommentDialogOpen} onOpenChange={setIsCommentDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Agrega un comentario sobre el estado del socio inactivo.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                if (editingComment) {
                  updateMemberComment(editingComment.memberId, editingComment.comment)
                }
              }}
            >
              Guardar Comentario
