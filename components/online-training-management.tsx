...
const appointmentStatusLabel: Record<string, string> = { confirmed: "Confirmada", completed: "Realizada", no_show: "No se presentó", not_completed: "No se realizó", cancelled: "Cancelada" };
const appointmentStatusClass: Record<string, string> = { confirmed: "bg-blue-100 text-blue-800", completed: "bg-emerald-100 text-emerald-800", no_show: "bg-amber-100 text-amber-900", not_completed: "bg-rose-100 text-rose-800", cancelled: "bg-slate-200 text-slate-700" };
...
const currentRoutinesWithEnd = (routines: RoutineInfo[], today: string) => routines.filter((routine) => routineIsActiveOn(routine, today) && routine.valid_until);
const hasRoutineAfterCurrent = (routines: RoutineInfo[], today: string) => currentRoutinesWithEnd(routines, today).some((currentRoutine) => routines.some((routine) => routine.id !== currentRoutine.id && Boolean(routine.valid_from && routine.valid_from > currentRoutine.valid_until!)));
const needsNextPeriodRoutine = (routines: RoutineInfo[], today: string) => currentRoutinesWithEnd(routines, today).some((currentRoutine) => !routines.some((routine) => routine.id !== currentRoutine.id && Boolean(routine.valid_from && routine.valid_from > currentRoutine.valid_until!)));

const whatsappPhone = (value: string | null) => {
  const digits = (value ?? "").replace(/[\s().+-]/g, "");
  const international = digits.startsWith("00") ? digits.slice(2) : digits;
  const phone = /^09\d{7}$/.test(international) ? "598" + international.slice(1)
    : /^9\d{7}$/.test(international) ? "598" + international : international;
...
export function OnlineTrainingManagement({ gymId }: { gymId: string }) {
  ...
  const [validityPeriod, setValidityPeriod] = useState<ValidityPeriodFilter>("all");
  ...
  const [appointmentOutcomeTarget, setAppointmentOutcomeTarget] = useState<{ appointment: Appointment; client: OnlineClient } | null>(null);
  const [appointmentOutcome, setAppointmentOutcome] = useState<"completed" | "no_show" | "not_completed">("completed");
  const [appointmentOutcomeNotes, setAppointmentOutcomeNotes] = useState("");
  const [savingAppointmentOutcome, setSavingAppointmentOutcome] = useState(false);
  ...
  const load = async () => {
    ...
    const [configResult, clientsResult, appointmentsResult] = await Promise.all([
      ...
      supabase.from("online_training_appointments").select("id, client_id, starts_at, ends_at, status, outcome_notes, outcome_recorded_at").eq("gym_id", gymId).order("starts_at", { ascending: false }),
    ]);
  ...
  const openAppointmentOutcome = (appointment: Appointment, client: OnlineClient, outcome: "completed" | "no_show" | "not_completed") => {
    setAppointmentOutcomeTarget({ appointment, client }); setAppointmentOutcome(outcome); setAppointmentOutcomeNotes("");
  };
  const saveAppointmentOutcome = async () => {
    if (!appointmentOutcomeTarget) return;
    setSavingAppointmentOutcome(true);
    const { appointment } = appointmentOutcomeTarget;
    const update = { status: appointmentOutcome, outcome_notes: appointmentOutcomeNotes.trim() || null, outcome_recorded_at: new Date().toISOString() };
    const { error } = await supabase.from("online_training_appointments").update(update).eq("id", appointment.id).eq("gym_id", gymId);
    setSavingAppointmentOutcome(false);
    if (error) return setMessage("No pudimos registrar el resultado. Verificá que la migración de historial de reuniones esté ejecutada.");
    setAppointments((current) => current.map((item) => item.id === appointment.id ? { ...item, ...update } : item));
    setAppointmentOutcomeTarget(null); setMessage("Resultado de la reunión guardado en el historial.");
  };
  ...
  const filtered = useMemo(() => {
    ...
    const validityBounds = (() => {
      ...
      const endOfYear = (year: number) => localDate(new Date(year, 11, 31));
      const startOfMonth = (date: Date) => localDate(new Date(date.getFullYear(), date.getMonth(), 1));
      const endOfMonth = (date: Date) => localDate(new Date(date.getFullYear(), date.getMonth() + 1, 0));
      const monthsAgo = (months: number) => { const date = new Date(now); date.setMonth(date.getMonth() - months); return localDate(date); };
      if (validityPeriod === "current_month") return { from: startOfMonth(now), until: endOfMonth(now) };
      if (validityPeriod === "last_month") return { from: monthsAgo(1), until: today };
      if (validityPeriod === "last_3_months") return { from: monthsAgo(3), until: today };
      if (validityPeriod === "current_year") return { from: startOfYear(now.getFullYear()), until: endOfYear(now.getFullYear()) };
      if (validityPeriod === "previous_year") return { from: startOfYear(now.getFullYear() - 1), until: endOfYear(now.getFullYear() - 1) };
      if (validityPeriod === "last_12_months") return { from: monthsAgo(12), until: today };
      if (validityPeriod === "custom") return { from: expiresFrom, until: expiresUntil };
      return { from: "", until: "" };
    })();
    const soon = new Date(); soon.setDate(soon.getDate() + 7);
    const soonEnd = localDate(soon);
    return clients.filter((client) => {
      ...
      const matchesExpiryRange = !validityBounds.from && !validityBounds.until ? true : routines.some((routine) => {
        ...
        const endsAt = routine.valid_until ?? "9999-12-31";
        return (!validityBounds.until || startsAt <= validityBounds.until) && (!validityBounds.from || endsAt >= validityBounds.from);
      });
      const matchesStatus = statusFilter === "all" || client.status === statusFilter;
      const matchesAppointment = appointmentFilter === "all" || !nextAppointment(client.id);
      return matchesSearch && matchesRoutineFilter && matchesExpiryRange && matchesStatus && matchesAppointment;
    });
  }, [appointmentFilter, appointments, clients, expiresFrom, expiresUntil, query, routineFilter, routinesByClient, statusFilter, validityPeriod]);
  const shownClients = useMemo(() => filtered.slice(0, visibleClientCount), [filtered, visibleClientCount]);
  useEffect(() => { setVisibleClientCount(10); }, [appointmentFilter, expiresFrom, expiresUntil, query, routineFilter, statusFilter, validityPeriod]);
  const clearFilters = () => { setRoutineFilter("all"); setStatusFilter("all"); setAppointmentFilter("all"); setValidityPeriod("all"); setExpiresFrom(""); setExpiresUntil(""); };
  ...
  const sendRoutineWhatsApp = (client: OnlineClient, routineLink: string) => {
  ...
  const upcomingSchedule = useMemo(() => appointments
    .filter((appointment) => appointment.status === "confirmed" && new Date(appointment.starts_at) >= new Date())
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-3xl border-slate-200 bg-white text-slate-950 shadow-sm">
        ...
        <CardContent>{loading ? <p className="py-8 text-center text-slate-500">Cargando...</p> : filtered.length === 0 ? <div className="py-10 text-center text-slate-500"><ClipboardList className="mx-auto mb-3 h-8 w-8" />Todavía no hay solicitudes.</div> : <div className="space-y-4">{shownClients.map((client) => {
          ...
          return <article key={client.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <details className="group mt-4 rounded-xl border border-slate-200 bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-bold text-blue-700 hover:bg-blue-50"><span>Ver ficha, rutinas y notas</span><span className="text-xs font-medium text-slate-500">Desplegar</span></summary><div className="border-t border-slate-200 p-3">
              ...
              <div className="mt-4"><p className="mb-2 text-sm font-bold text-slate-800">Historial del cliente</p>{clientHistory(client.id, clientRoutines).length ? <div className="space-y-2">{clientHistory(client.id, clientRoutines).map((entry) => entry.kind === "routine" ? (() => { const routine = entry.routine; const routineLink = routine.public_share_token && routine.public_link_enabled ? `${window.location.origin}/rutina/${routine.public_share_token}` : null; return <div key={`routine-${routine.id}`} className="rounded-xl border border-violet-200 bg-violet-50/60 p-3"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-950">Rutina · {routine.name}</p><Badge className="bg-violet-100 text-violet-800">Rutina</Badge></div><p className="mt-1 text-sm text-slate-600">{routine.valid_from ? `Desde ${dateLabel(routine.valid_from)}` : "Sin fecha de inicio"}{routine.valid_until ? ` · hasta ${dateLabel(routine.valid_until)}` : ""}</p></div><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => editRoutine(client, routine)} className="border-blue-200 bg-white text-blue-700 hover:bg-blue-50"><Pencil className="mr-1.5 h-3.5 w-3.5" />Editar</Button>{routineLink ? <><Button type="button" size="sm" variant="outline" onClick={() => void copy(routineLink, "Link de rutina copiado.")} className="border-blue-200 bg-white text-blue-700 hover:bg-blue-50"><ExternalLink className="mr-1.5 h-3.5 w-3.5" />Copiar link</Button><Button type="button" size="sm" disabled={!client.phone} onClick={() => sendRoutineWhatsApp(client, routineLink)} className="bg-emerald-600 text-white hover:bg-emerald-700 disabled:bg-slate-300"><MessageCircle className="mr-1.5 h-3.5 w-3.5" />Enviar por WhatsApp</Button></> : null}<Button type="button" size="sm" variant="outline" disabled={deletingRoutineId === routine.id} onClick={() => void deleteRoutine(client, routine)} className="border-rose-200 bg-white text-rose-700 hover:bg-rose-50"><Trash2 className="mr-1.5 h-3.5 w-3.5" />{deletingRoutineId === routine.id ? "Eliminando..." : "Eliminar"}</Button></div></div></div>; })() : <div key={`appointment-${entry.appointment.id}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold text-slate-900">Reunión · {dateLabel(entry.appointment.starts_at)} · {new Date(entry.appointment.starts_at).toLocaleTimeString("es-UY", { hour: "2-digit", minute: "2-digit" })}</p><Badge className={appointmentStatusClass[entry.appointment.status] ?? "bg-slate-200 text-slate-700"}>{appointmentStatusLabel[entry.appointment.status] ?? entry.appointment.status}</Badge></div>{entry.appointment.outcome_notes ? <p className="mt-2 text-sm text-slate-700">{entry.appointment.outcome_notes}</p> : null}{entry.appointment.outcome_recorded_at ? <p className="mt-2 text-xs text-slate-500">Registrado el {dateLabel(entry.appointment.outcome_recorded_at)} · {new Date(entry.appointment.outcome_recorded_at).toLocaleTimeString("es-UY", { hour: "2-digit", minute: "2-digit" })}</p> : null}</div>)}</div> : <p className="rounded-xl border border-dashed border-slate-300 p-4 text-center text-sm text-slate-500">Todavía no hay eventos registrados.</p>}</div>
        ...
        })}<div className="flex flex-wrap items-center justify-end gap-2 border-t border-slate-200 pt-4"><p className="mr-2 text-sm font-medium text-slate-500">Mostrando {shownClients.length} de {filtered.length} clientes</p>{visibleClientCount > 10 && <Button type="button" size="sm" variant="outline" onClick={() => setVisibleClientCount((count) => Math.max(10, count - 10))} className="border-slate-300 bg-white text-slate-700 hover:bg-slate-50">Ver 10 menos</Button>}{shownClients.length < filtered.length && <Button type="button" size="sm" onClick={() => setVisibleClientCount((count) => count + 10)} className="bg-blue-600 text-white hover:bg-blue-700">Ver 10 más</Button>}</div></div>}</CardContent>
      </Card>
      ...
      <Dialog open={isScheduleOpen} onOpenChange={setIsScheduleOpen}>
        <DialogContent className="max-h-[88vh] overflow-y-auto rounded-3xl border-slate-200 bg-white p-6 text-slate-950 sm:max-w-3xl">
          ...
          {upcomingSchedule.length ? <div className="mt-3 space-y-3">{upcomingSchedule.map(({ appointment, client }) => <article key={appointment.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-600">{dateLabel(appointment.starts_at)} · {new Date(appointment.starts_at).toLocaleTimeString("es-UY", { hour: "2-digit", minute: "2-digit" })}</p><h3 className="mt-1 text-lg font-black text-slate-950">{client.full_name}</h3><p className="mt-1 text-sm text-slate-600">Cédula: {client.cedula}</p></div><span className="w-fit rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">Confirmada</span></div><div className="mt-4 grid gap-2 border-t border-slate-200 pt-3 sm:grid-cols-2">{client.phone ? <a href={`tel:${client.phone}`} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-blue-50 hover:text-blue-700"><Phone className="h-4 w-4 text-blue-600" />{client.phone}</a> : <span className="rounded-xl bg-white px-3 py-2 text-sm text-slate-500">Sin teléfono registrado</span>}<a href={`mailto:${client.email}`} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-blue-50 hover:text-blue-700"><Mail className="h-4 w-4 text-blue-600" />{client.email}</a></div><div className="mt-3 flex flex-wrap gap-2"><Button type="button" size="sm" onClick={() => openAppointmentOutcome(appointment, client, "completed")} className="bg-emerald-600 text-white hover:bg-emerald-700"><CheckCircle2 className="mr-1.5 h-4 w-4" />Marcar realizada</Button><Button type="button" size="sm" variant="outline" onClick={() => openAppointmentOutcome(appointment, client, "no_show")} className="border-amber-300 bg-white text-amber-800 hover:bg-amber-50"><X className="mr-1.5 h-4 w-4" />No se presentó / no se realizó</Button></div></article>)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-600"><CalendarDays className="mx-auto h-7 w-7 text-slate-400" /><p className="mt-3 font-semibold">No hay reuniones futuras agendadas.</p></div>}
        </DialogContent>
      </Dialog>
      <Dialog open={Boolean(appointmentOutcomeTarget)} onOpenChange={(open) => !open && setAppointmentOutcomeTarget(null)}>
        <DialogContent className="rounded-3xl border-slate-200 bg-white p-6 text-slate-950 sm:max-w-lg"><DialogHeader><DialogTitle>Registrar resultado de reunión</DialogTitle><DialogDescription className="text-slate-600">{appointmentOutcomeTarget ? `${appointmentOutcomeTarget.client.full_name} · ${dateLabel(appointmentOutcomeTarget.appointment.starts_at)}` : ""}</DialogDescription></DialogHeader><div className="space-y-4 pt-3"><div className="grid gap-2"><p className="text-sm font-bold text-slate-800">Resultado</p><Select value={appointmentOutcome} onValueChange={(value) => setAppointmentOutcome(value as "completed" | "no_show" | "not_completed")}><SelectTrigger className="border-slate-300 bg-white text-slate-950"><SelectValue /></SelectTrigger><SelectContent className="bg-white text-slate-950"><SelectItem value="completed">Reunión realizada</SelectItem><SelectItem value="no_show">No se presentó</SelectItem><SelectItem value="not_completed">No se realizó</SelectItem></SelectContent></Select></div><div><p className="mb-2 text-sm font-bold text-slate-800">Comentario o motivo <span className="font-normal text-slate-500">(opcional)</span></p><Textarea value={appointmentOutcomeNotes} onChange={(event) => setAppointmentOutcomeNotes(event.target.value)} placeholder={appointmentOutcome === "completed" ? "Ej.: Revisamos avances y se ajustó la rutina." : "Ej.: No pudo asistir por trabajo; reprogramar."} className="min-h-28 border-slate-300 bg-white text-slate-950" /></div><Button type="button" disabled={savingAppointmentOutcome} onClick={() => void saveAppointmentOutcome()} className="w-full bg-blue-600 text-white hover:bg-blue-700"><Save className="mr-2 h-4 w-4" />{savingAppointmentOutcome ? "Guardando..." : "Guardar en historial"}</Button></div></DialogContent>
      </Dialog>
    </div>
  );
}
