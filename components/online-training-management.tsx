        ...
        <CardContent>{loading ? <p className="py-8 text-center text-slate-500">Cargando...</p> : filtered.length === 0 ? <div className="py-10 text-center text-slate-500"><ClipboardList className="mx-auto mb-3 h-8 w-8" />Todavía no hay solicitudes.</div> : <div className="space-y-4">{shownClients.map((client) => {
          ...
          return <article key={client.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <details className="group mt-4 rounded-xl border border-slate-200 bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-bold text-blue-700 hover:bg-blue-50"><span>Ver ficha, rutinas y notas</span><span className="text-xs font-medium text-slate-500">Desplegar</span></summary><div className="border-t border-slate-200 p-3">
              ...
              <div className="mt-4"><p className="mb-2 text-sm font-bold text-slate-800">Historial de reuniones</p>{appointmentHistory(client.id).length ? <div className="space-y-2">{appointmentHistory(client.id).map((history) => <div key={history.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold text-slate-900">{dateLabel(history.starts_at)} · {new Date(history.starts_at).toLocaleTimeString("es-UY", { hour: "2-digit", minute: "2-digit" })}</p><Badge className={appointmentStatusClass[history.status] ?? "bg-slate-200 text-slate-700"}>{appointmentStatusLabel[history.status] ?? history.status}</Badge></div>{history.outcome_notes ? <p className="mt-2 text-sm text-slate-700">{history.outcome_notes}</p> : null}{history.outcome_recorded_at ? <p className="mt-2 text-xs text-slate-500">Registrado el {dateLabel(history.outcome_recorded_at)} · {new Date(history.outcome_recorded_at).toLocaleTimeString("es-UY", { hour: "2-digit", minute: "2-digit" })}</p> : null}</div>)}</div> : <p className="rounded-xl border border-dashed border-slate-300 p-4 text-center text-sm text-slate-500">Todavía no hay reuniones registradas.</p>}</div>
