...
type ValidityPeriodFilter = "all" | "last_month" | "last_3_months" | "current_year" | "previous_year" | "last_12_months" | "custom";
...
export function OnlineTrainingManagement({ gymId }: { gymId: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    const validityBounds = (() => {
      ...
      if (validityPeriod === "current_year") return { from: startOfYear(now.getFullYear()), until: today };
    ...
    return clients.filter((client) => {
      ...
      const matchesExpiryRange = !validityBounds.from && !validityBounds.until ? true : routines.some((routine) => Boolean(routine.valid_until && (!validityBounds.from || routine.valid_until >= validityBounds.from) && (!validityBounds.until || routine.valid_until <= validityBounds.until)));
