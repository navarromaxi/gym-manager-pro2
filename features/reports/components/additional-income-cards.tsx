    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg border p-3"><p className="text-sm text-muted-foreground">Detalle por origen</p><div className="mt-2 flex flex-wrap gap-2"><Badge variant="secondary">TuPase: {sourceTotals.TuPase.count}</Badge><Badge variant="secondary">PaseLibre: {sourceTotals.PaseLibre.count} · {formatCurrency(sourceTotals.PaseLibre.amount)}</Badge><Badge variant="secondary">Otros: {sourceTotals.Otros.count} · {formatCurrency(sourceTotals.Otros.amount)}</Badge></div></div>
        </div>
        {monthlyStats.length > 0 ? <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Mes</TableHead><TableHead>Total (cant. · monto)</TableHead><TableHead>TuPase</TableHead><TableHead>PaseLibre</TableHead><TableHead>Otros</TableHead></TableRow></TableHeader><TableBody>{monthlyStats.map((row) => <TableRow key={row.month}><TableCell className="font-medium">{row.month}</TableCell><TableCell>{formatCountAmount(row.total)}</TableCell><TableCell>{formatCountAmount(row.TuPase)}</TableCell><TableCell>{formatCountAmount(row.PaseLibre)}</TableCell><TableCell>{formatCountAmount(row.Otros)}</TableCell></TableRow>)}</TableBody></Table></div> : <p className="text-sm text-muted-foreground">No se registran pagos únicos en el período seleccionado.</p>}
      </CardContent>
    </Card>
    <div className="grid gap-6 lg:grid-cols-2"><Card>
      ...
      <CardContent><div className="space-y-2">{last6MonthsIncome.map((monthData) => <div key={monthData.month} className="flex items-center justify-between rounded-lg border px-3 py-2"><span className="font-medium">{monthData.month}</span><span className="text-base font-bold text-green-600">${monthData.income.toLocaleString()}</span></div>)}</div></CardContent>
    </Card></div>
  </>;
}
