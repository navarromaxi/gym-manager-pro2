...
export function PersonalizedRoutineBuilder({ gymId, members, initialRoutine, createAsNew = false, defaultMember, onSaved, onCancel }: { gymId: string; members: RoutineMemberSearch[]; initialRoutine?: CreatedPersonalizedRoutine; createAsNew?: boolean; defaultMember?: RoutineMemberSearch; onSaved: (routine: CreatedPersonalizedRoutine) => void; onCancel: () => void }) {
  const isEditingExisting = Boolean(initialRoutine && !createAsNew);
  ...
  const save = async () => {
    ...
    const routine: CreatedPersonalizedRoutine = { id: isEditingExisting ? initialRoutine!.id : `${Date.now()}`, name: name.trim(), description: description.trim(), targetAudience: "Rutina personalizada", difficulty: "Intermedio", duration, exercises: getDayExercises(cleanedPlan, "monday"), createdDate: isEditingExisting ? initialRoutine?.createdDate ?? new Date().toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10), createdBy: initialRoutine?.createdBy ?? "Usuario Actual", memberId: member.id, weeklyPlan: cleanedPlan, validFrom: validFrom || null, validUntil: validUntil || null, dayIntensities, planCycle, cyclePlan: cleanedCyclePlan };
