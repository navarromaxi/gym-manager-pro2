...
export default function OnlineTrainingIntakePage({ params }: { params: Promise<{ gymId: string }> }) {
  ...
  const next = async () => {
    ...
    if (step === 1) {
      ...
      setLoading(false);
      if (!response?.ok) return setError(result && "error" in result ? result.error ?? "No pudimos verificar tu inscripción." : "No pudimos verificar tu inscripción.");
      if (result?.exists) { setExistingRegistration(result as ExistingRegistration); return; }
    }
