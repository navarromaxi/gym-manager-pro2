...
export default function OnlineTrainingIntakePage({ params }: { params: Promise<{ gymId: string }> }) {
  ...
  const next = async () => {
    const missingStepOne = !form.fullName || !form.email || !form.phone;
  ...
  const resumePayment = async () => {
    ...
    const payment = await fetch("/api/online-training/subscription", {
      ...
      body: JSON.stringify({ gymId, cedula: form.cedula }),
    ...
    setError("No pudimos abrir el pago. Intentá nuevamente en unos minutos.");
  };
  const submit = async (event: FormEvent) => {
    ...
    if (!form.fullName || !form.cedula || !form.email || !form.phone || !form.goal || !form.trainingPlace || !form.experience || !form.injuries || !form.availableTime || !form.weeklyDays || !form.source || (form.source === "Otro" && !form.sourceDetail) || !form.consent) {
      ...
      return setStep(!form.fullName || !form.email || !form.phone ? 1 : !form.goal || !form.trainingPlace || !form.experience || !form.injuries || !form.availableTime || !form.weeklyDays ? 2 : 3);
    }
    setSubmitting(true); setError("");
    ...
    const registration = response ? await response.json().catch(() => null) as ExistingRegistration | { exists?: false; error?: string } | null : null;
    if (!response?.ok) { setSubmitting(false); return setError(registration && "error" in registration ? registration.error ?? "No pudimos verificar tu inscripción." : "No pudimos verificar tu inscripción."); }
    if (registration?.exists) { setSubmitting(false); setExistingRegistration(registration as ExistingRegistration); return; }
      ...
      <section className="mx-auto max-w-6xl">
        <header className="relative overflow-hidden rounded-[2.5rem] bg-[#07162d] px-6 py-9 text-white shadow-[0_30px_85px_rgba(8,30,65,.25)] sm:px-10 sm:py-12">
          ...
          <div className="relative flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl border border-cyan-200/25 bg-white/10 text-cyan-200"><Sparkles className="h-5 w-5" /></span><p className="text-xs font-extrabold uppercase tracking-[0.24em] text-cyan-200">Plan personal online</p></div><h1 className="mt-6 max-w-xl text-4xl font-black leading-[.98] tracking-tight sm:text-6xl">Entrená con dirección, no con dudas.</h1><p className="mt-5 max-w-xl text-base leading-7 text-slate-200 sm:text-lg">Contanos un poco sobre vos y un profesor real prepara tu rutina mensual, con videos para entender cada ejercicio.</p></div>
        ...
        <form onSubmit={submit} className="relative mt-5 overflow-hidden rounded-[2.25rem] border border-blue-100 bg-white shadow-[0_24px_70px_rgba(30,64,175,.15)]">
          <div className="border-b border-slate-200 bg-[linear-gradient(120deg,_#f8fbff,_#eff6ff)] px-5 py-5 sm:px-8"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-blue-700">{existingRegistration ? "Inscripción encontrada" : `Paso ${step} de 3`}</p><h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950">{existingRegistration ? "Ya tenemos tus datos" : step === 1 ? "Para conocerte" : step === 2 ? "Tu entrenamiento" : "Confirmación y pago"}</h2></div><span className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-sm font-extrabold text-blue-700">{existingRegistration ? "Listo" : step === 1 ? "33%" : step === 2 ? "66%" : "100%"}</span></div>{!existingRegistration && <div className="mt-5 flex gap-2">{[1, 2, 3].map((number) => <button key={number} type="button" onClick={() => setStep(number)} className={`h-2.5 flex-1 rounded-full transition ${number <= step ? "bg-blue-600" : "bg-slate-200"}`} aria-label={`Ir al paso ${number}`} />)}</div>}</div>
          <div className="p-5 sm:p-8">
            {existingRegistration ? <div className="space-y-5"><div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-cyan-50 p-5"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-emerald-700">Hola, {existingRegistration.clientName}</p><h3 className="mt-2 text-xl font-black text-slate-950">{existingRegistration.status === "active" || existingRegistration.status === "payment_due" ? "Tu plan ya está activo." : "Tu inscripción ya está registrada."}</h3><p className="mt-2 text-sm leading-6 text-slate-700">{existingRegistration.status === "active" || existingRegistration.status === "payment_due" ? "No hace falta que completes el formulario nuevamente. Desde acá podés ver tu rutina o gestionar la reunión de seguimiento." : "Conservamos tus datos para que no tengas que completar todo otra vez. Sólo falta continuar o regularizar el pago de la suscripción."}</p></div><div className="flex flex-col gap-3 sm:flex-row">{(existingRegistration.status === "active" || existingRegistration.status === "payment_due") ? <>{existingRegistration.routinePath && <a href={existingRegistration.routinePath} className="inline-flex min-h-12 flex-1 items-center justify-center rounded-xl bg-blue-600 px-5 font-bold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700">Ver mi rutina</a>}<a href={`https://www.pymessistemas.com/gestionar-reunion.html?cedula=${encodeURIComponent(form.cedula)}`} className="inline-flex min-h-12 flex-1 items-center justify-center rounded-xl border border-emerald-300 bg-emerald-50 px-5 font-bold text-emerald-800 transition hover:bg-emerald-100">Gestionar mi reunión</a></> : <Button type="button" disabled={submitting} onClick={() => void resumePayment()} className="min-h-12 flex-1 bg-blue-600 text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700">{submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{submitting ? "Abriendo pago..." : "Completar pago en Mercado Pago"}</Button>}<Button type="button" variant="outline" onClick={() => { setExistingRegistration(null); setError(""); }} className="min-h-12 border-slate-300 bg-white text-slate-800">Usar otra cédula</Button></div></div> : <><div className="mb-7 flex flex-col gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"><span className="font-bold text-slate-900">Te lleva menos de 2 minutos.</span><span className="flex items-center gap-2 text-slate-600"><CheckCircle2 className="h-4 w-4 text-emerald-600" />Después seguís al pago seguro.</span></div>
            {step === 1 && <div className="space-y-5"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-950"><b>Estás en la inscripción segura de RutinaOnline.</b> Primero conocemos tus objetivos; la cédula se solicita recién antes del pago para registrar tu plan.</div><div className="grid gap-5 sm:grid-cols-2"><div><Label className={labelClass}>Nombre y apellido</Label><Input required value={form.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder="Tu nombre completo" className={fieldClass} /></div><div><Label className={labelClass}>Email</Label><Input required type="email" value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="tu@email.com" className={fieldClass} /></div><div className="sm:col-span-2"><Label className={labelClass}>Teléfono / WhatsApp</Label><Input required type="tel" value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="Ej.: 099 123 456" className={fieldClass} /></div></div></div>}
