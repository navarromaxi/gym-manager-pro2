...
export default function PublicRoutinePage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#dbeafe,_#f8fafc_45%,_#eefbf6)] px-4 py-6 sm:px-6 sm:py-10"><div className="mx-auto max-w-6xl">
    ...
    {tab === "routine" ? <article className="public-routine-card overflow-hidden rounded-3xl border border-white bg-white shadow-[0_18px_45px_rgba(15,23,42,0.1)]">
      ...
      {isOnlineTraining ? <div className="border-t bg-gradient-to-r from-blue-50 to-indigo-50 p-5 sm:px-7"><div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center"><div><p className="font-black text-slate-950">¿Querés revisar tu progreso?</p><p className="mt-1 text-sm text-slate-600">Agendá tu reunión con el profe para el próximo mes.</p></div><a href="/entrenamiento/entrenamiento_online/agenda" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700"><CalendarCheck2 className="h-4 w-4" />Agendar mi reunión</a></div></div> : null}
      ...
      <div className="rounded-3xl border border-blue-200 bg-white p-5 shadow-sm sm:flex sm:items-center sm:justify-between"><div><p className="font-black text-slate-950">Tu seguimiento también importa</p><p className="mt-1 text-sm text-slate-600">Reservá una reunión para conversar sobre tu próximo mes.</p></div><a href="/entrenamiento/entrenamiento_online/agenda" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-blue-700 sm:mt-0"><CalendarCheck2 className="h-4 w-4" />Agendar mi reunión</a></div>
    </section>}
  </div></main>;
}
